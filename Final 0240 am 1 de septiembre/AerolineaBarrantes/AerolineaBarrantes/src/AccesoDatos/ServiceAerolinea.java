...
public class ServiceAerolinea extends Service{
        ...
        public List<Vuelos> listarVuelos(String n) throws SQLException {
        ResultSet rs = null;
        vuelos = new ArrayList(); 
        CallableStatement pstmt = null;
        Vuelos losVuelos= null;
        try {
            getConnection();
            pstmt = connection.prepareCall(ListarVuelos);
            pstmt.registerOutParameter(1, OracleTypes.CURSOR);
            pstmt.setString(2, n);
            pstmt.execute();
            rs = (ResultSet) pstmt.getObject(1);
            while (rs.next()) {
                List<Ciudades> origen=(List<Ciudades>)listarCiudadesPorId(rs.getString("origen"));
                List<Ciudades> destino=(List<Ciudades>)listarCiudadesPorId(rs.getString("destino"));
                Ciudades origen1=origen.get(0);
                Ciudades destino1=destino.get(0);
                losVuelos = new Vuelos(rs.getString("numero_vuelo"),
                        origen1,
                        destino1,
                        rs.getInt("costo"),
                        rs.getString("tiempo"),
